...
class VAELR(BaseAE):
    ...
    def forward(self, inputs: BaseDataset, **kwargs):
        ...
        params = torch.concat((mu, log_var)).flatten()
        ...
        with torch.no_grad():
            encoder_cycle_output = self.encoder(recon_x)
            recon_params = torch.concat((encoder_cycle_output.embedding, encoder_cycle_output.log_covariance)).flatten()

        loss, recon_loss, kld, lat_reg = self.loss_function(recon_x, x, mu, log_var, z, params, recon_params)

        output = ModelOutput(
            recon_loss=recon_loss,
            reg_loss=kld,
            lat_reg_loss = lat_reg,
            loss=loss,
            recon_x=recon_x,
            z=z,
        )
    ...
    def loss_function(self, recon_x, x, mu, log_var, z, params, recon_params):
        ...
        lat_reg = F.mse_loss(params, recon_params)

        return (recon_loss + KLD + lat_reg).mean(dim=0), recon_loss.mean(dim=0), KLD.mean(dim=0), lat_reg.mean(dim=0)
